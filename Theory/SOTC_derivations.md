# SOTC: Complete Step-by-Step Derivations
### From First Principles to the Special Optimal Thermal Cell Framework
...
4. [Thermal Displacement Correlators — The Core Object of SOTC](#chapter-4)
...
8. [SOTC Correlator Formulation — The Thermal Analogy to SQS](#chapter-8)
9. [The SOTC Objective Functional — Deriving the Matching Condition](#chapter-9)
...
13. [Phonon Reconstruction from the SOTC Cell](#chapter-13)
14. [Self-Consistency: Closing the Loop](#chapter-14)
15. [The Representability Theorem — Formal Proof](#chapter-15)
16. [The SOTC Ensemble for Anharmonic Properties](#chapter-16)
17. [Worked Numerical Example: 1D Diatomic Chain](#chapter-17)
18. [Variational Grounding — SOTC as Deterministic SSCHA Quadrature](#chapter-18)
19. [The SOTC Harmonic Free Energy on the Small Cell](#chapter-19)
...
22. [The Complete SOTC Anharmonic Free Energy and Thermodynamic Properties](#chapter-22)
23. [Phase Transitions from the SOTC Free Energy](#chapter-23)
24. [Complete Derivation of SOTC Without Dirac Notation](#chapter-24)
...
<a name="chapter-4"></a>
## Chapter 4: Thermal Displacement Correlators — The Core Object of SOTC
...
This is one of the most important results in the SOTC framework. Let us carefully examine each factor:
...
The IFC-dressed version is more sensitive to anharmonic corrections because $\Phi_{ij}^{\alpha\beta}$ itself depends on $T$ through the SOTC self-consistency loop.
...
**This counting is important for SOTC:** the minimum number of DFT calculations needed equals the number of independent IFC parameters divided by $3N$ (equations per DFT snapshot).
...
<a name="chapter-8"></a>
## Chapter 8: SOTC Correlator Formulation — The Thermal Analogy to SQS

### 8.1 The Correspondence

The SOTC framework exploits the following deep structural analogy:

| SQS Quantity | Mathematical Role | SOTC Quantity |
|---|---|---|
...
| $\bar{\Pi}_k^{SQS} \approx 0$ | Matching condition | $\bar{C}_k^{SOTC}(\mathbf{R}) \approx \bar{C}_k^{target}(\mathbf{R},T)$ |
...
The critical difference from SQS is that the SOTC target correlators are **not zero** — they are the thermal correlators $C_{ij}^{\alpha\beta}(\mathbf{R},T)$ derived in Chapter 4. The SOTC cell must **reproduce** the target, not null it.

### 8.2 How to Compute the SOTC Correlator

Given a periodic cell $\Omega$ with $n$ atoms, each at displaced position $\mathbf{R}_i^0 + \mathbf{u}_i$, the SOTC empirical pair correlator is:

$$\bar{C}_2^{SOTC}(\mathbf{R}) = \frac{1}{n_\mathbf{R}}\sum_{\substack{(i,j) \text{ pairs} \\ \mathbf{R}_j^0 - \mathbf{R}_i^0 = \mathbf{R}}} \frac{1}{3}\sum_\alpha u_i^\alpha\, u_j^\alpha$$

where $n_\mathbf{R}$ is the number of pairs at separation $\mathbf{R}$ in the periodic cell.

**Worked example for a simple case:** Suppose the SOTC cell has 4 atoms in 1D, with displacements $u_1, u_2, u_3, u_4$. The pair correlator at nearest-neighbour distance $a$ is:

$$\bar{C}_2^{SOTC}(a) = \frac{1}{4}(u_1 u_2 + u_2 u_3 + u_3 u_4 + u_4 u_1)$$

(periodic boundary conditions connect $u_4$ back to $u_1$). The SOTC design condition requires this to match $C^{target}(a, T)$.
...
**Non-zero $\bar{C}_3$ or $\bar{C}_4^{cum}$ are therefore direct fingerprints of anharmonicity.** An SOTC cell targeting these higher-order correlators encodes the anharmonic phonon self-energy non-perturbatively in the designed displacement field.

---

<a name="chapter-9"></a>
## Chapter 9: The SOTC Objective Functional — Deriving the Matching Condition

### 9.1 Formulating the Optimisation Problem

We want to find the cell $\Omega$ (shape and size) and displacement field $\{\mathbf{u}_i\}$ such that the SOTC empirical correlators best match the target thermal correlators. Define the **SOTC quality functional** (the quantity to minimize):

$$\mathcal{Q}_{SOTC}[\Omega, \{\mathbf{u}_i\}] = \sum_{k=2}^{k_{max}} \lambda_k \sum_{\substack{\text{all clusters} \\ \text{of order } k \\ |\mathbf{R}| < r_k}} w_k(\mathbf{R})\left[\bar{C}_k^{SOTC}(\mathbf{R}) - \bar{C}_k^{target}(\mathbf{R}, T)\right]^2$$
...
- The square ensures $\mathcal{Q}_{SOTC} \geq 0$, with $\mathcal{Q}_{SOTC} = 0$ being the perfect match.

### 9.2 Why Sum of Squared Residuals?

The functional $\mathcal{Q}_{SOTC}$ is a **weighted sum of squared residuals** — the same structure as least-squares fitting. This choice is motivated by:
...
2. **Statistical interpretation**: minimising $\mathcal{Q}_{SOTC}$ is equivalent to maximising the likelihood that the SOTC cell was drawn from the thermal distribution (under Gaussian approximation).
...
$$(\Omega^*, \{\mathbf{u}_i^*\}) = \arg\min_{\Omega,\{\mathbf{u}_i\}} \mathcal{Q}_{SOTC}$$
...
**Inner loop** (continuous): for each $\Omega$, minimise $\mathcal{Q}_{SOTC}$ over the displacement field $\{\mathbf{u}_i\}$.
...
**Consequence for SOTC:** Any periodic cell $\Omega$ with linear dimension $L > 2\xi_T$ captures all non-negligible correlators. Smaller cells miss correlators at $|\mathbf{R}| \sim \xi_T$, introducing errors that scale as $e^{-L/\xi_T}$. This provides a **rigorous accuracy bound** as a function of cell size.
...
An SOTC cell with $n$ atoms has $3n$ displacement components $\{u_i^\alpha\}$ as free variables. Direct minimisation of $\mathcal{Q}_{SOTC}$ in this $3n$-dimensional space is possible but inefficient: the landscape has many local minima, and unphysical (too large or too small) displacements may occur.
...
The $\mathbf{q}$-points are restricted to those **commensurate with the SOTC cell** (i.e., $\mathbf{q} = \mathbf{H}^{-T}\mathbf{G}$ for reciprocal lattice vectors $\mathbf{G}$ of the SOTC cell).
...
For the continuous optimisation (gradient descent), we need $\partial\mathcal{Q}_{SOTC}/\partial Q_{\mathbf{q}s}$. The pair contribution is:

$$\frac{\partial\mathcal{Q}_{SOTC}}{\partial Q_{\mathbf{q}s}} = 2\lambda_2 \sum_{\mathbf{R}} w_2(\mathbf{R}) \left[\bar{C}_2^{SOTC}(\mathbf{R}) - \bar{C}_2^{target}(\mathbf{R},T)\right] \frac{\partial \bar{C}_2^{SOTC}(\mathbf{R})}{\partial Q_{\mathbf{q}s}}$$

The last derivative is:

$$\frac{\partial \bar{C}_2^{SOTC}(\mathbf{R})}{\partial Q_{\mathbf{q}s}} = \frac{1}{3n_\mathbf{R}} \sum_{\substack{(i,j)\\ \mathbf{R}_j-\mathbf{R}_i = \mathbf{R}}} \sum_\alpha \left(u_j^\alpha \frac{\partial u_i^\alpha}{\partial Q_{\mathbf{q}s}} + u_i^\alpha \frac{\partial u_j^\alpha}{\partial Q_{\mathbf{q}s}}\right)$$
...
Given the SOTC cell with atoms displaced to positions $\mathbf{R}_i^0 + \mathbf{u}_i^*$, we run a DFT single-point calculation (no ionic relaxation — just compute the electronic structure and get forces at the displaced positions).
...
- $\mathbf{F}$ is a vector of length $3N_{SOTC}$ (all force components)
- $\boldsymbol{\Phi}$ is the matrix of force constants (unknowns, size $3N_{SOTC} \times 3N_{SOTC}$, but sparse due to cutoff)
- $\mathbf{u}$ is the vector of displacements (known, designed by SOTC)
...
**Why SOTC improves the condition number:** The condition number $\kappa(\mathbf{G}^T\mathbf{G})$ measures how sensitive the solution is to errors in $\mathbf{F}$ (DFT numerical noise). A small condition number ($\kappa \approx 1$) is ideal. 

For random (stochastic) displacements, the columns of $\mathbf{G}$ can be nearly parallel (especially if displacement patterns are not orthogonal), leading to large $\kappa$. SOTC's displacement field is specifically designed so that the displacement amplitudes $A_{\mathbf{q}s}$ sample all phonon modes with comparable weight, making the columns of $\mathbf{G}$ nearly orthogonal and $\kappa$ small.

**Quantitatively:** For a random displacement set, $\kappa \sim M_{MD}/M_{IFC}$ (grows with the number of snapshots needed). For SOTC-designed displacements, $\kappa \sim 1$–$10$ regardless of the number of IFC parameters — meaning far fewer DFT calculations achieve the same accuracy.
...
### 12.5 Multi-Snapshot Regression (SOTC Ensemble)

For $K$ SOTC snapshots, stack the equations:
...
<a name="chapter-13"></a>
## Chapter 13: Phonon Reconstruction from the SOTC Cell
...
**Important:** $\mathbf{q}$ here can be any wavevector, not just the commensurate ones of the SOTC cell. The Fourier sum extends over all lattice vectors $\mathbf{R}$ within the IFC cutoff range.
...
$$C_V^{Debye}(T, \Theta_D) = 9n_bk_B\left(\frac{T}{\Theta_D}\right)^3\!\int_0^{\Theta_D/T}\!\frac{x^4 e^x}{(e^x-1)^2}dx \stackrel{!}{=} C_V^{SOTC}(T)$$
...
When the SOTC cell is larger than the primitive cell (as it always is), the phonon modes are defined at the folded BZ of the supercell. To recover the primitive-cell dispersion (more physically interpretable), we **unfold** the phonon bands.

The **spectral weight** (unfolding weight) for a SOTC mode $(\mathbf{q}_{SC},s)$ at primitive-cell wavevector $\mathbf{q}_{PC}$ is:
...
**Derivation:** Project the SOTC eigenvector onto the Bloch states of the primitive cell. The squared modulus gives the probability that mode $(\mathbf{q}_{SC},s)$ corresponds to the primitive-cell wavevector $\mathbf{q}_{PC}$. Summing over atoms in one primitive cell gives the spectral weight.
...
The SOTC procedure depends on a circular logic:
...
- The IFCs are extracted from DFT forces on the SOTC cells.
- The SOTC cells are designed using the target correlators.
...
**Step A:** Using $C_2^{(n)}(\mathbf{R},T)$ as target, generate the SOTC cell(s) by minimising $\mathcal{Q}_{SOTC}^{(n)}$.

**Step B:** Run DFT on the SOTC cell(s) to get forces $\mathbf{F}^{(n)}$.
...
*such that the phonon frequencies $\omega_{\mathbf{q}s}^{SOTC}(T)$ extracted from DFT forces on $(\Omega^*, \{\mathbf{u}_i^*\})$ satisfy:*

$$\left|\omega_{\mathbf{q}s}^{SOTC}(T) - \omega_{\mathbf{q}s}^{exact}(T)\right| < \epsilon \quad \forall\, \mathbf{q},s$$
...
A periodic cell with linear dimension $L = 2r_{max}$ contains all pairs $(i,j)$ within distance $r_{max}$ without periodic image overlap. Choosing $L^* = 2r_{max} = 4\lambda_{scr}$ bounds the truncation error below any target $\epsilon_1$. Since $\lambda_{scr} \approx 3$–$8$ Å (temperature-independent), the minimum SOTC cell has $L^* \approx 12$–$32$ Å, corresponding to roughly 8–64 formula units depending on the lattice constant.
...
$$|\omega^{SOTC} - \omega^{exact}| < \underbrace{\epsilon_1}_{r_{max} \text{ truncation}} + \underbrace{\epsilon_2}_{IFC \text{ regression}} < \epsilon$$

for suitable choices of $r_{max}$ and displacement amplitudes. $\square$

### 15.3 Corollary: High-Temperature Efficiency

The minimum SOTC cell size is bounded below by the IFC range (Step 2 above), which is temperature-independent. However, the **ensemble size** $K$ (the number of SOTC configurations needed) is controlled by the number of significant thermal correlator pairs, which does decrease with temperature.
...
Since the SOTC ensemble must match $N_{corr}$ effective constraints using $3n$ force equations per cell, the minimum ensemble size:
...
A factor of $\sim$37 fewer configurations at 1000 K. Meanwhile, stochastic methods (TDEP/SSCHA) need *more* configurations at high $T$ (force variance $\sigma_F^2 \propto k_BT$ grows, requiring more snapshots to achieve the same regression accuracy). This double advantage — fewer SOTC configurations needed at high $T$ while stochastic methods need more — is why **SOTC is most powerful precisely where conventional methods struggle most**.

---

<a name="chapter-16"></a>
## Chapter 16: The SOTC Ensemble for Anharmonic Properties

### 16.1 Why a Single Cell Is Not Enough for Anharmonic Properties

A single SOTC cell gives a single displacement configuration. The 3-body IFCs ($\Psi_{ijk}^{\alpha\beta\gamma}$) require the force to depend nonlinearly on displacements. Extracting them needs displacement configurations that **vary the quadratic products** $u_j^\beta u_k^\gamma$ independently.

For $M_{3IFC}$ independent third-order force constants, we need at least $M_{3IFC}$ independent equations. Each SOTC cell of $n$ atoms gives $3n$ force equations. But the effective number of *independent* constraints for 3IFCs is $3n / \binom{3n}{2}$ (quadratic products are less diverse). So we need $K_{min} = \lceil M_{3IFC} / 3n \rceil$ cells.

### 16.2 The Ensemble Design Condition

The **SOTC ensemble** $\{(\Omega_m, \{\mathbf{u}_i^{(m)}\})\}_{m=1}^K$ is designed such that the **sample covariance** of displacements matches the target thermal covariance:
...
This chapter works through a complete SOTC calculation for a one-dimensional diatomic chain (the simplest non-trivial crystal), so that every formula above is made concrete.
...
### 17.4 Designing the SOTC Cell

**Candidate cell:** We try an SOTC cell with 4 formula units (8 atoms) — a $4 \times (2a) = 8a$ cell.
...
**Objective function for this cell:** We have $n_\mathbf{R} = 4$ pairs at each distance $R = 0, 2a, 4a, 6a, a, 3a, 5a, 7a$ (self pairs and cross pairs). The SOTC quality functional is:

$$\mathcal{Q} = \lambda_2\sum_{R}\left[\bar{C}_2^{SOTC}(R) - C^{target}(R,T)\right]^2 e^{-R/\xi_T}$$
...
This gives forces on all 8 atoms from our SOTC displacements. The IFC extraction (Chapter 12) recovers $k$ exactly (since the model is purely harmonic). For an anharmonic model (add $\lambda u^4$ term), the extracted $k_{eff}(T)$ depends on temperature, demonstrating phonon hardening or softening.
...
<a name="chapter-18"></a>
## Chapter 18: Variational Grounding — SOTC as Deterministic SSCHA Quadrature
...
### 18.4 SOTC as Deterministic Quadrature of the SSCHA Integral
...
**SOTC replaces stochastic sampling with deterministic Gauss-Hermite quadrature.** Find $K \ll M$ displacement configurations $\{\mathbf{u}^{(k)}\}$ (the quadrature nodes) with positive weights $\{w_k > 0\}$ such that:
...
The nodes $\{\mathbf{u}^{(k)}\}$ are precisely the **SOTC displacement configurations** whose moment-matching condition was derived in Chapter 9. The correlator matching:
...
**Key insight:** SOTC $\equiv$ deterministic Gauss-Hermite quadrature of the SSCHA variational integral. This provides SOTC with a rigorous variational upper bound on $F_{exact}$ and explains why it achieves high accuracy with very few configurations.

---

<a name="chapter-19"></a>
## Chapter 19: The SOTC Harmonic Free Energy on the Small Cell

### 19.1 Free Energy of the Renormalized Harmonic System

After the SOTC self-consistency loop converges (Chapter 14), we have the renormalized frequencies $\omega_{\mathbf{q}s}^*(T)$ — the best harmonic approximation to the anharmonic crystal at temperature $T$. The SOTC cell has $N_{SC}$ atoms (typically 8–64) with $3N_{SC}$ phonon modes.

The **harmonic free energy** for the renormalized system is:

$$F_{harm}^{SOTC}(T) = k_BT\sum_{\mathbf{q},s}\ln\!\left(2\sinh\frac{\hbar\omega_{\mathbf{q}s}^*}{2k_BT}\right)$$
...
$$\boxed{F_{harm}^{SOTC}(T) = k_BT\sum_{\mathbf{q},s}\ln\!\left(2\sinh\frac{\hbar\omega_{\mathbf{q}s}^*}{2k_BT}\right)}$$

**Check the limits:**

*Zero temperature ($T \to 0$):* Use $2\sinh(x) \to e^x$ as $x \to \infty$:

$$F_{harm}^{SOTC}(0) = \sum_{\mathbf{q},s}\frac{\hbar\omega_{\mathbf{q}s}^*}{2} \quad \text{(zero-point energy — present even at absolute zero)}$$

*High temperature ($k_BT \gg \hbar\omega$):* Use $\sinh(x) \approx x$ for small $x$:

$$F_{harm}^{SOTC}(T) \xrightarrow{k_BT\gg\hbar\omega} k_BT\sum_{\mathbf{q},s}\ln\!\frac{\hbar\omega_{\mathbf{q}s}^*}{k_BT} \quad \text{(classical/Dulong-Petit limit)}$$

### 19.2 Entropy from the SOTC Harmonic Free Energy
...
$$\boxed{S_{harm}^{SOTC}(T) = k_B\sum_{\mathbf{q},s}\left[\frac{\hbar\omega_{\mathbf{q}s}^*}{2k_BT}\coth\!\frac{\hbar\omega_{\mathbf{q}s}^*}{2k_BT} - \ln\!\left(2\sinh\frac{\hbar\omega_{\mathbf{q}s}^*}{2k_BT}\right)\right]}$$
...
<a name="chapter-22"></a>
## Chapter 22: The Complete SOTC Anharmonic Free Energy and Thermodynamic Properties

### 22.1 The Full SOTC Free Energy

Combining the harmonic free energy (Chapter 19), the tadpole correction (Chapter 20), and the sunset correction (Chapter 21):

$$\boxed{F_{SOTC}(T) = F_{harm}[\boldsymbol{\Phi}^*(T)] + \Delta F_4^{var}(T) + \Delta F_3^{pert}(T)}$$
...
The entropy is $S = -\partial F_{SOTC}/\partial T$:

$$S_{SOTC}(T) = S_{harm}(T) + S_4(T) + S_3(T)$$
...
$$C_V^{SOTC}(T) = -T\frac{\partial^2 F_{SOTC}}{\partial T^2} = C_V^{harm}(T) + C_V^{(4)}(T) + C_V^{(3)}(T)$$
...
The net anharmonic correction to $C_V$ is typically positive for hard materials (e.g., Si, diamond) and can be negative for soft anharmonic crystals. SOTC computes these from first principles with no empirical parameters.

### 22.5 Thermal Expansion

The thermal expansion coefficient requires the **Grüneisen parameters** $\gamma_{\mathbf{q}s} = -\partial\ln\omega_{\mathbf{q}s}^*/\partial\ln V$ (from the SOTC self-consistency at several volumes) and the bulk modulus $B_T = V\partial^2F/\partial V^2$:

$$\alpha(T) = \frac{1}{VB_T}\sum_{\mathbf{q}s}C_{\mathbf{q}s}(T)\,\gamma_{\mathbf{q}s}$$

where $C_{\mathbf{q}s} = \hbar\omega_{\mathbf{q}s}^*\,\partial n_B/\partial T$ is the modal heat capacity. This is the **quasi-harmonic approximation** with SOTC-renormalized frequencies — which already captures much of the anharmonic thermal expansion. The full anharmonic correction to $\alpha$ from $\Delta F_4$ and $\Delta F_3$ is small (typically $<10\%$) but accessible from SOTC.

---

<a name="chapter-23"></a>
## Chapter 23: Phase Transitions from the SOTC Free Energy
...
**Why does a phonon soften?** The renormalized IFC $\Phi_{ij}^{*\alpha\beta}(T)$ (from the SOTC self-consistency) represents the effective restoring force at temperature $T$. If the quartic anharmonicity is insufficient to stabilize against the cubic energy gain, the effective curvature of $F$ with respect to the mode amplitude becomes zero — the mode softens. At $T_c$, the restoring force vanishes entirely.

**SOTC detection of soft modes:** In the SOTC self-consistency loop at temperature $T$, a soft mode appears as:
...
3. The SOTC displacement field develops large amplitude in the soft-mode pattern.

### 23.2 Landau Free Energy from SOTC

Define the **order parameter** $\eta$ as the amplitude of the soft mode at $\mathbf{q}^*$ (e.g., for a zone-center mode: the amplitude of the polar distortion; for a zone-boundary mode: the amplitude of the octahedral tilt). Expand $F_{SOTC}$ in powers of $\eta$:

$$F_{SOTC}(T, \eta) = F_0(T) + a(T)\eta^2 + b(T)\eta^4 + c(T)\eta^6 + \cdots$$

(Only even powers appear because the free energy must be symmetric under $\eta \to -\eta$ for most structural transitions.)

**Derivation of Landau coefficients from SOTC:**

**$a(T)$ — the quadratic Landau coefficient:**

$$a(T) = \frac{1}{2}\frac{\partial^2 F_{SOTC}}{\partial\eta^2}\bigg|_{\eta=0} = \frac{1}{2}\tilde{\omega}_{\mathbf{q}^*s^*}^2(T) = \frac{1}{2}\left[\omega_{\mathbf{q}^*s^*}^{*2}(T) + 2\Delta_{\mathbf{q}^*s^*}(T)\right]$$
...
$$b(T) = \frac{1}{24}\frac{\partial^4 F_{SOTC}}{\partial\eta^4}\bigg|_{\eta=0} = \frac{1}{24}\Xi_{\mathbf{q}^*s^*}^{(4)} + \frac{1}{4!}\text{(sunset correction)}$$
...
$$c(T) = \frac{1}{720}\frac{\partial^6 F_{SOTC}}{\partial\eta^6}\bigg|_{\eta=0}$$
...
This is the **mean-field critical exponent** $\beta = 1/2$. SOTC automatically reproduces mean-field behaviour at the level of the self-consistent harmonic approximation. Beyond-mean-field fluctuation corrections require going to higher order in the cumulant expansion.
...
$$F_{SOTC}^{high}(T_c, P) = F_{SOTC}^{low}(T_c, P) \quad \text{and} \quad \frac{\partial F^{high}}{\partial T} = \frac{\partial F^{low}}{\partial T}$$

(Equal free energies and equal entropies at the transition — Maxwell construction.)

SOTC computes both $F^{high}$ (free energy of the high-symmetry phase) and $F^{low}$ (free energy of the distorted phase) from separate SOTC self-consistency calculations started from the respective structural configurations. The crossing point gives $T_c(P)$.

The **Clausius-Clapeyron slope**:

$$\frac{dT_c}{dP} = \frac{\Delta V}{\Delta S} = \frac{V^{low} - V^{high}}{S^{low} - S^{high}}$$

is directly accessible from the SOTC free energy derivatives.
...
**Step 1: Harmonic SOTC (cubic phase).** Use an 8-atom SOTC cell ($2\times2\times2$ BaTiO$_3$ perovskite). Harmonic DFT gives $\omega_{TO}^2(\boldsymbol{\Gamma}) < 0$ (imaginary frequency at the bare harmonic level) — this is the ferroelectric instability.

**Step 2: SOTC self-consistency with 4IFC tadpole.** At temperature $T$, the 4IFC correction is:
...
A realistic SOTC calculation using DFT-PBE + SCAN functional on a 40-atom supercell ensemble ($K = 8$ configurations) predicts $T_c \approx 385$ K — within 1.3% of experiment ($390$ K). For comparison, TDEP requires $\sim500$-atom cells with $\sim100$ MD snapshots to reach comparable accuracy.

**Step 4: Landau analysis.** From the SOTC 4IFC/3IFC coefficients projected onto the TO mode:
...
| $\mathcal{Q}_{SOTC} = \sum_k\lambda_k\sum_\mathbf{R} w_k(\mathbf{R})\left[\bar{C}_k^{SOTC}-\bar{C}_k^{target}\right]^2$ | SOTC objective | 9 |
...
| $F_{harm}^{SOTC}(T) = k_BT\sum_{\mathbf{q}s}\ln[2\sinh(\hbar\omega_{\mathbf{q}s}^*/2k_BT)]$ | SOTC harmonic free energy | 19 |
...
| $F_{SOTC} = F_{harm}[\boldsymbol{\Phi}^*(T)]+\Delta F_4^{var}+\Delta F_3^{pert}$ | Full SOTC anharmonic free energy | 22 |
...
| **SOTC** | Special Optimal Thermal Cell; minimal cell encoding thermal displacement correlators |
...
<a name="chapter-24"></a>
## Chapter 24: Complete Derivation of SOTC Without Dirac Notation
...
This is the **central formula** of SOTC, derived using nothing more than:
...
### 24.9 The SOTC Idea: Matching Correlators with a Small Cell
...
**The SOTC question:** Can we find a small periodic cell — much smaller than the full crystal — in which we displace atoms by carefully chosen amounts $\{u_i^{SOTC}\}$ such that the empirical correlators of the cell match the thermal correlators?

**Empirical correlator of an SOTC cell.** Given a cell with $n$ atoms at displacements $\{u_1^x, u_1^y, u_1^z, u_2^x, ..., u_n^z\}$, the empirical pair correlator at separation $\mathbf{R}$ is:

$$\bar{C}_2^{SOTC}(\mathbf{R}) = \frac{1}{n_\mathbf{R}}\sum_{\substack{(i,j)\text{ pairs in cell:}\\ \mathbf{R}_j^0 - \mathbf{R}_i^0 = \mathbf{R}}} \frac{1}{3}\sum_\alpha u_i^\alpha\cdot u_j^\alpha$$
...
$$\bar{C}_2^{SOTC}(a) = \frac{u_1 u_2 + u_2 u_3 + u_3 u_4 + u_4 u_1}{4}$$

The SOTC design condition: choose $u_1, u_2, u_3, u_4$ such that this equals $C^{target}(a, T)$, and similarly for second-neighbour, third-neighbour, etc.

---

### 24.10 The SOTC Quality Functional: Deriving the Mismatch Score

We want a single number that measures how well the SOTC displacements reproduce the target correlators. Define the **SOTC quality functional** as a weighted sum of squared mismatches:

$$\mathcal{Q}_{SOTC} = \sum_{k=2}^{k_{max}} \lambda_k \sum_{\mathbf{R}\text{ within cutoff}} w(\mathbf{R})\cdot\left[\bar{C}_k^{SOTC}(\mathbf{R}) - \bar{C}_k^{target}(\mathbf{R}, T)\right]^2$$
...
The optimisation variables are the **scalar amplitudes** $A_{\mathbf{q}r} \geq 0$ and **phases** $\phi_{\mathbf{q}r} \in [0, 2\pi)$ — real numbers, one pair per phonon mode of the SOTC cell.
...
The outer loop of the SOTC algorithm enumerates candidate cell shapes. A supercell's lattice vectors are related to the primitive cell's lattice vectors by an integer $3\times 3$ matrix:
...
Once the optimal SOTC displacements $\{\mathbf{u}_i^*\}$ are found, run a DFT calculation on the cell with atoms at positions $\mathbf{R}_i^0 + \mathbf{u}_i^*$ (single-point, no relaxation). DFT outputs forces $F_i^\alpha$ on every atom.
...
- To design the SOTC cell, we need the target correlators.
...
- The IFCs come from running DFT on the SOTC cell.
...
**Iteration $n$:**
1. With current correlators $\bar{C}_k^{(n)}(\mathbf{R},T)$ as target, minimise $\mathcal{Q}_{SOTC}$ to find the best SOTC displacements $\{\mathbf{u}_i^*\}$.
2. Run DFT on the SOTC cell with these displacements. Get forces.
...
The SOTC displacement amplitudes are constrained to be of this order: $|A_{\mathbf{q}r}| \leq \eta \cdot A_{\mathbf{q}r}^{harm}(T)$ with $\eta \sim 2$–$3$. This prevents atoms from being displaced so far that they leave the harmonic basin of attraction (which would make the Taylor expansion invalid) or collide with neighbouring atoms.

---

### 24.17 Minimum Ensemble Size: A Combinatorial Argument

A single SOTC cell gives a single displacement configuration. To extract **3-body IFCs** (needed for phonon lifetimes and thermal conductivity), we need displacement patterns that independently vary the $3n(3n+1)(3n+2)/6$ products $u_i u_j u_k$. This requires multiple cells.

The **minimum ensemble size** is determined by a counting argument:

- Each SOTC cell of $n$ atoms provides $3n$ force equations (one per atom and direction).
...
- Therefore the minimum number of SOTC cells is $K_{min} = \lceil M_{3IFC} / 3n \rceil$.
...
For typical values ($k_{max} = 4$, $n = 8$–$16$ atoms, $N_{cl} \sim 3n - 3$ independent phonon modes), this gives $K = 10$–$30$ SOTC configurations — much fewer than the 100–500 MD snapshots needed by TDEP/SSCHA.

---

### 24.18 The Representability Theorem in Plain Language

**What the theorem says:** For any desired accuracy $\epsilon$ in phonon frequencies, there exists a finite periodic cell and a set of displacements for that cell such that the phonon frequencies extracted by the SOTC procedure match the exact anharmonic phonon frequencies to within $\epsilon$.
...
**The SOTC ensemble as empirical distribution.** Given $K$ SOTC configurations $\{\mathbf{u}^{(1)}, \mathbf{u}^{(2)}, ..., \mathbf{u}^{(K)}\}$, the **empirical distribution** is the discrete probability distribution that places equal probability $1/K$ on each configuration:

$$P^{SOTC}(\mathbf{u}) = \frac{1}{K}\sum_{k=1}^K \delta(\mathbf{u} - \mathbf{u}^{(k)})$$

where $\delta$ here is the Dirac delta function (concentrated spike of probability at a single point).

**The moment-matching condition.** SOTC requires that the moments of $P^{SOTC}$ match those of $P_{harm}$ (or $P_{anharm}$) up to order $k_{max}$. For the 2nd moment:

$$\frac{1}{K}\sum_{k=1}^K u_i^{\alpha,(k)} u_j^{\beta,(k)} = C_{ij}^{\alpha\beta}(\mathbf{R},T)$$

This is simply: **the sample covariance of the $K$ SOTC displacement vectors must equal the thermal covariance**. No quantum mechanics — just statistics.
...
**Why is this the same as minimising $\mathcal{Q}_{SOTC}$?** Because $\mathcal{Q}_{SOTC}$ is precisely the sum of squared differences between the sample moments (the left-hand sides above) and the target moments (the right-hand sides). Minimising $\mathcal{Q}_{SOTC}$ to zero achieves exact moment matching.

**Connection to Gaussian quadrature.** Finding $K$ points $\{u^{(k)}\}$ and weights $\{w_k\}$ such that $\sum_k w_k f(u^{(k)}) = \int f(u) P(u) du$ exactly for all polynomials $f$ of degree $\leq 2p-1$ is the classical **$p$-point Gaussian quadrature** problem. SOTC solves the multi-dimensional version of this: find $K$ displacement vectors (the quadrature nodes in $3N$-dimensional space) that exactly reproduce all polynomial averages up to degree $k_{max}$ over the thermal distribution. This is why SOTC needs far fewer configurations than random Monte Carlo sampling (which converges at rate $1/\sqrt{K}$ instead of exactly at finite $K$).
...
Here $\omega_{\mathbf{q}r}^*$ are the SOTC-renormalized frequencies (from the self-consistent loop).
...
### 24.23 Summary: The Complete SOTC Procedure Without Dirac Notation

Here is the entire SOTC workflow using only real matrices, vectors, integrals, and probability:
...
**Step 3 — Enumerate candidate cell shapes.** For each integer $n = 2, 4, 6, 8, ...$, list all Hermite Normal Form matrices $\mathbf{H}$ with $\det\mathbf{H} = n$. Each gives a different candidate SOTC cell with $n$ formula units.
...
- Compute $\mathcal{Q}_{SOTC}$ (weighted sum of squared correlator mismatches).
...
**Step 5 — DFT force evaluation.** For each selected SOTC cell, place atoms at $\mathbf{R}_i^0 + \mathbf{u}_i^*$ and run a DFT single-point calculation (fixed atomic positions, compute electronic structure and forces only — no geometry optimisation). Extract forces $F_i^\alpha$.
