...
In the harmonic limit, $\bar{C}_3 = 0$ (odd moments vanish by symmetry) and $\bar{C}_4$ factorizes. **Non-zero $\bar{C}_3$ and non-factorized $\bar{C}_4$ are the fingerprints of anharmonicity.** The SQTC framework can target these explicitly, encoding third- and fourth-order interatomic force constants (3IFCs, 4IFCs) in the small-cell design.

### 2.3 The SQTC Objective Function

Let $\Omega$ denote a candidate periodic cell with $n$ atoms (where $n \ll N_{supercell}$) and let $\{\mathbf{u}_i\}$ be the set of atomic displacements assigned to those atoms. The **SQTC quality functional** is:

$$\mathcal{Q}_{SQTC}[\Omega, \{\mathbf{u}_i\}] = \sum_{k=2}^{k_{max}} \lambda_k \sum_{\substack{\text{clusters} \\ |\mathbf{R}| < r_k}} w_k(\mathbf{R})\left[\bar{C}_k^{SQTC}(\mathbf{R}) - \bar{C}_k^{target}(\mathbf{R}, T)\right]^2$$
...
where $v_s$ is the average sound velocity, $\tau_{ph}$ is the average phonon lifetime (from Grüneisen parameter $\gamma$), and $\bar{\omega}$ is the mean phonon frequency. As $T$ increases, $\xi_T$ decreases — the thermal disorder is *more local* — which means smaller cells are sufficient at higher temperatures. **SQTC is therefore most efficient precisely where conventional methods are most expensive.**

The SQTC optimization problem is:

$$\left(\Omega^*, \{\mathbf{u}_i^*\}\right) = \arg\min_{\Omega, \{\mathbf{u}_i\}} \mathcal{Q}_{SQTC}[\Omega, \{\mathbf{u}_i\}]$$
...
To make the optimization tractable, we expand atomic displacements in a physically motivated basis. For atom $i$ in the SQTC cell:
...
The optimization adjusts $Q_{\mathbf{q}s}$ away from harmonic values to encode anharmonic correlators. This constitutes a **non-perturbative thermal displacement field** encoded in the SQTC cell.

---

## 3. The SQTC Algorithm
...
Enumerate candidate SQTC cells using **Hermite Normal Form** (HNF) decomposition of the supercell transformation matrix $\mathbf{H}$:

$$\mathbf{A}_{SQTC} = \mathbf{H}\, \mathbf{A}_{prim}, \quad \mathbf{H} \in HNF(n)$$
...
For each candidate cell $\Omega$, solve the minimization of $\mathcal{Q}_{SQTC}$ over the normal-coordinate amplitudes $\{Q_{\mathbf{q}s}\}$ using:
...
for each candidate cell Ω with n atoms:
    ...
    record (Q_SQTC, Ω, {u_i}) for this candidate
select top-M cells with lowest Q_SQTC
```

### Step 4 — DFT Force Evaluation

Run DFT single-point calculations (no ionic relaxation) on the selected SQTC cells with atoms at positions $\mathbf{R}_i^0 + \mathbf{u}_i^*$. Extract forces $\mathbf{F}_i$ from DFT.

For an SQTC ensemble of $M$ cells, the effective IFCs are obtained by solving the linear system:

$$\sum_{j,\beta} \Phi_{ij}^{\alpha\beta}\, u_j^\beta = -F_i^\alpha$$

via least-squares regression (identical to the TDEP force-constant extraction), but now using the *designed* SQTC displacements instead of MD snapshots.
...
For dilute alloy/defect systems, apply **phonon band unfolding** (Boykin-Klimeck formalism) from the SQTC BZ to the primitive cell BZ, recovering the spectral function:

$$A(\mathbf{q}, \omega, T) = \sum_s P(\mathbf{q} | \mathbf{q}_{SQTC}, s)\, \delta\!\left[\omega - \omega_{\mathbf{q}s}(T)\right]$$

where $P(\mathbf{q} | \mathbf{q}_{SQTC}, s)$ is the unfolding weight (spectral weight).

### Step 6 — Self-Consistent Refinement

The procedure is closed into a **self-consistent loop**:

$$\bar{C}_k^{(0)} \xrightarrow{\text{Step 1}} \text{SQTC}^{(0)} \xrightarrow{\text{DFT}} \Phi^{(0)} \xrightarrow{\text{phonons}} \omega^{(0)}, \bar{C}_k^{(1)} \xrightarrow{\text{Step 1}} \cdots$$
...
## 4. The SQTC Ensemble for Anharmonic Properties

### 4.1 Constructing the Ensemble

For thermodynamic properties requiring sampling of phase space (free energy, thermal conductivity), a single SQTC cell is insufficient. We propose the **SQTC ensemble** of $M$ cells:

$$\mathcal{E}_{SQTC} = \left\{(\Omega_m, \{\mathbf{u}_i^{(m)}\})\right\}_{m=1}^{M}$$
...
$$\left\langle f(\{\mathbf{u}_i\})\right\rangle_{SQTC} \equiv \frac{1}{M}\sum_m f\!\left(\{\mathbf{u}_i^{(m)}\}\right) \approx \left\langle f(\{\mathbf{u}_i\})\right\rangle_T$$
...
For $k_{max} = 4$ (quartic) and $N_{cluster} = 3n-3$ (3N-3 independent modes in a cell of $n$ atoms), typical values give $M \sim 10$–$30$ SQTC cells, each with $n \sim 8$–$16$ atoms — compared to $\sim 100$–$500$ MD snapshots on cells of $>$50 atoms in TDEP/SSCHA.

### 4.3 Thermal Conductivity from SQTC

Phonon-phonon scattering rates require 3IFCs. From the SQTC ensemble, 3IFCs are extracted via the extended regression:
...
The purposefully designed displacements in SQTC cells ensure the quadratic displacement products $u_j^\beta u_k^\gamma$ sample the space of 3IFC coefficients efficiently, improving the condition number of the regression by a factor of $\sim \sqrt{M_{SQTC}/M_{MD}} \cdot \sqrt{N_{SQTC}/N_{MD}}$ relative to stochastic approaches.

---

## 5. Formal Connections and Analogies

| Concept | SQS (Chemical) | SQTC (Thermal) |
|---|---|---|
...
**Limit 3: Einstein oscillators.** Each atom vibrates independently. $C_2(\mathbf{R}, T) = \delta_{\mathbf{R},0}\frac{k_BT}{M\omega_E^2}$. A single-atom SQTC cell (the primitive cell) with displacement amplitude $u = \sqrt{k_BT/M\omega_E^2}$ is exact.

**Limit 4: Dilute alloy.** Chemical disorder handled by SQS; dynamical disorder by SQTC. The combined framework is **SQS-SQTC**: first apply SQS to fix chemical order, then apply SQTC to the resulting structure for thermal displacements. This correctly captures impurity-phonon scattering in the dilute limit.

### 5.2 Relationship to TDEP

TDEP generates displacements stochastically from a trial harmonic Hamiltonian and refines IFCs from DFT forces. SQTC replaces stochastic sampling with **deterministic optimal sampling**: the displacements are chosen to maximally reduce $\mathcal{Q}_{SQTC}$ given the cell size. TDEP is the $M \to \infty$ limit of the SQTC ensemble with random (non-optimal) displacements.

### 5.3 Relationship to SSCHA

SSCHA minimizes the variational free energy $\mathcal{F}[\mathcal{D}]$ over trial harmonic density matrices $\mathcal{D}$. SQTC implicitly targets the same minimum but encodes the optimal density matrix *in coordinate space* via the designed displacement cells. SSCHA's stochastic integration over phase space is replaced by SQTC's discrete but optimal quadrature.
...
- $N_{SQTC}$: atoms in SQTC cell, $M_{SQTC}$: cells in SQTC ensemble

The conventional DFT cost scales as $\mathcal{C}_{conv} \sim M_{conv} \cdot N_{conv}^3$.

The SQTC cost scales as $\mathcal{C}_{SQTC} \sim M_{SQTC} \cdot N_{SQTC}^3$.

The **SQTC speedup factor** is:

$$\frac{\mathcal{C}_{conv}}{\mathcal{C}_{SQTC}} = \frac{M_{conv}}{M_{SQTC}} \cdot \left(\frac{N_{conv}}{N_{SQTC}}\right)^3$$

For representative values: $N_{conv}/N_{SQTC} \sim 4$–$8$, $M_{conv}/M_{SQTC} \sim 10$–$50$, giving speedup factors of **640$\times$ to 25,600$\times$** — several orders of magnitude.

More conservatively, accounting for the SQTC optimization overhead (negligible compared to DFT), the practical speedup is expected to be **100$\times$ to 1000$\times$**.

---

## 7. Theoretical Underpinning: The Phonon Correlation Functional Theorem

**Theorem (SQTC Representability):** *Let $\omega_{\mathbf{q}s}(T)$ be the exact temperature-renormalized phonon frequencies of an anharmonic crystal. For any $\epsilon > 0$ and spatial cutoff $r_{max}$, there exists a periodic cell $\Omega^*$ with $n^*$ atoms and displacement field $\{\mathbf{u}_i^*\}$ such that the dynamical matrix $D(\mathbf{q}, T)$ reconstructed from the IFCs of $(\Omega^*, \{\mathbf{u}_i^*\})$ satisfies:*

$$\left|\omega_{\mathbf{q}s}^{SQTC}(T) - \omega_{\mathbf{q}s}(T)\right| < \epsilon \quad \forall\, \mathbf{q}, s$$
...
**Corollary:** At high temperature, $\xi_T \propto T^{-1}$ (from the Grüneisen relation), so $r_{max}$ and consequently $n^*$ *decrease* with increasing temperature. SQTC is *more* accurate at high $T$ for a fixed cell size.

---

## 8. Extensions

### 8.1 Magnetic Systems: Spin-SQTC

At high temperature near a magnetic transition, spin fluctuations renormalize phonons via spin-lattice coupling. Extend the SQTC correlators to include spin-displacement correlators:

$$\bar{C}_2^{spin-ph}(\mathbf{R}, T) = \langle S_i^z\, u_j^\alpha \rangle_T$$

Cells are designed to encode both displacement amplitudes and spin configurations simultaneously, producing a **magnon-phonon coupled SQTC**.

### 8.2 Polar Materials: Dipole-Corrected SQTC
...
Augment the SQTC objective with a **dipole correction term**:

$$\mathcal{Q}_{SQTC}^{polar} = \mathcal{Q}_{SQTC} + \lambda_{LO} \left[\epsilon_\infty^{SQTC} - \epsilon_\infty^{ref}\right]^2 + \lambda_{Born}\left\|\mathbf{Z}^{SQTC} - \mathbf{Z}^{ref}\right\|^2$$
...
For systems with broken periodicity in one dimension (surfaces, 2D materials), the SQTC cell is designed with periodicity only in the in-plane directions. The correlation cutoff $r_{max}^{in-plane}$ is temperature-dependent while $r_{max}^{out-of-plane}$ is set by the slab thickness.

### 8.4 Pressure-Dependent SQTC

High pressure modifies both equilibrium positions and phonon dispersion. The SQTC cell is generated at the target $(T, P)$ using pressure-corrected equilibrium structures, enabling joint $(T, P)$ phase diagrams at minimal cost.
...
Phase 1 — Proof of Concept
  ├── [✓] SQTC runner: generates correlator-matched displacement fields,
...
Phase 2 — Anharmonic Extension
  ├── Implement 3IFC regression on SQTC ensemble
...
### Phase 3 — Chemical Disorder Coupling (SQS-SQTC) (Planned)

```
Phase 3 — Chemical Disorder Coupling (SQS-SQTC)
  ├── Combine SQS chemical decoration with SQTC displacements
  ├── Implement phonon band unfolding from SQTC to primitive BZ
...
Phase 4 — Public Code Release
  ...
  └── Web-based SQTC database for common systems
```
...
SQTC is expected to reduce the DFT cost of high-temperature phonon calculations by **two to four orders of magnitude**, opening the door to high-throughput thermodynamic screening and accurate phonon-mediated properties across the full composition-temperature-pressure phase space.
