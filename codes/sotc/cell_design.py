"""
SOTC cell design: Hermite Normal Form supercell enumeration and
...
Displacement expansion (Section 2.4 of SOTC_framework.md):
...
Simulated annealing minimises the SOTC quality functional:

    Q = Σ_R w(R) [C̄₂^SOTC(R) - C̄₂^target(R,T)]²
...
class HNFEnumerator:
    """
    Enumerate candidate SOTC supercells via Hermite Normal Form matrices.
...
class DisplacementOptimizer:
    """
    Optimise atomic displacement field {u_i} in an SOTC candidate cell to
    minimise the quality functional Q[C̄₂^SOTC − C̄₂^target].
    ...
    lambda_2 : float
        Weight of pair-correlator term (λ₂ in SOTC framework).
    """
    ...
    def generate_ensemble(
        ...
        """
        Generate an ensemble of M SOTC displacement configurations by
