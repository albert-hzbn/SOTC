"""
SOTC Post-processor  —  phonons + temperature-dependent properties.
...
Provides two public classes:
  ...
  SOTCRunLoader
      Reads force/displacement data from a completed SOTC work directory
      (POSCAR + OUTCAR in each snapshot sub-folder), re-fits the IFCs, and
      returns a ready-to-use :class:`SOTCPostProcessor`.

Typical usage::

    # Post-process a finished run without re-running SOTC
    loader = SOTCRunLoader(
    ...
    pp = loader.build()              # → SOTCPostProcessor
...
Alternatively, if you already have a PhononCalculator::
    ...
    from sotc.postprocessor import SOTCPostProcessor
    pp = SOTCPostProcessor(phonon_calc, label="fcc Al", elements=["Al"])
"""
...
try:
    from .constants import HBAR, KB, NA, AMU_TO_KG, EV_TO_J
    ...
    from sotc.constants import HBAR, KB, NA, AMU_TO_KG, EV_TO_J
...
class SOTCPostProcessor:
    ...
    T_design : float, optional
        SOTC design temperature [K]; shown on plot titles.
    """
    ...
    def compute_thermal_properties(
        ...
        # ── Compute frequency mesh ONCE ───────────────────────────────────────
        # This is the dominant cost; all T-dependent quantities are derived from
        # it via broadcasting instead of calling _all_frequencies per-T.
        all_omegas = self.calc._all_frequencies(q_mesh)   # (n_q, n_branches)
        ...
        # Positive-only modes used by Svib / Fvib (mirrors vibrational_entropy logic)
        pos_omegas = omegas_flat[omegas_flat > self.calc._imag_tol]  # (n_pos,)

        # ── T-independent scalars ─────────────────────────────────────────────
        ZPE_eV = float(np.sum(pos_omegas) * HBAR / 2.0 / n_q_pts / EV_TO_J)

        # Spectral T_D: max of the 3 acoustic branches at each q
        _max_ac = 0.0
        for _row in all_omegas:
            _p = _row[_row > self.calc._imag_tol]
            if len(_p) >= 3:
                _max_ac = max(_max_ac, np.sort(_p)[:3].max())
        TD_spectral = float(HBAR * _max_ac / KB)

        # ── Heat capacity — vectorised (n_T,) ─────────────────────────────────
        # Reuse the already-computed valid_mask / omega_use
        T_col   = T_arr[:, None]                            # (n_T, 1)
        x_all   = HBAR * omega_use[None, :] / (2.0 * KB * np.where(T_col < 1e-9, 1.0, T_col))
        x_c_all = np.clip(x_all, 0.0, 350.0)
        sinh_all  = np.sinh(x_c_all)
        sinh_safe = np.where(x_all < 1e-10, 1.0, sinh_all)  # avoid 0/0 at x=0
        cv_mode   = np.where(x_all < 1e-10, 1.0, (x_all / sinh_safe) ** 2)
        cv_mode   = np.where(valid_mask[None, :], cv_mode, 0.0)
        Cv_arr    = KB * NA * np.sum(cv_mode, axis=1) / n_q_pts
        Cv_arr    = np.where(T_arr < 1e-9, 0.0, Cv_arr)

        # ── Svib and Fvib — vectorised using positive modes only ──────────────
        x_pos   = HBAR * pos_omegas[None, :] / (2.0 * KB * np.where(T_col < 1e-9, 1.0, T_col))
        x_pos_c = np.clip(x_pos, 0.0, 350.0)
        # Svib = kB * NA * Σ [x/tanh(x) - ln(2 sinh(x))] / n_q
        tanh_pos  = np.tanh(x_pos_c)
        tanh_safe = np.where(x_pos < 1e-10, x_pos_c + 1e-30, tanh_pos)  # tanh≈x for small x
        Svib_arr  = KB * NA * np.sum(
            x_pos / tanh_safe - np.log(2.0 * np.sinh(x_pos_c)), axis=1
        ) / n_q_pts
        Svib_arr  = np.where(T_arr < 1e-9, 0.0, Svib_arr)
        # Fvib = kB*T * Σ ln(2 sinh(x)) / n_q  [eV]
        Fvib_arr  = (KB * np.where(T_arr < 1e-9, 1.0, T_arr)
                     * np.sum(np.log(2.0 * np.sinh(x_pos_c)), axis=1)
                     / n_q_pts / EV_TO_J)
        Fvib_arr  = np.where(T_arr < 1e-9, ZPE_eV, Fvib_arr)

        # ── Calorimetric T_D — requires per-T bisection (kept serial) ─────────
        TD_arr = np.array([
            self.calc._calorimetric_debye_temperature(float(cv), float(T))
            for cv, T in zip(Cv_arr, T_arr)
        ])

        # ── Mean-squared displacement — vectorised ────────────────────────────
        omega_safe = np.where(pos_mask, omega_use, np.inf)
        # x for MSD uses ALL modes (omega_use, not pos_omegas)
        x_msd   = HBAR * omega_use[None, :] / (2.0 * KB * np.where(T_col < 1e-9, 1.0, T_col))
        x_msd_c = np.clip(x_msd, 0.0, 350.0)
        # coth(x): safe for x≈0 (those modes are masked by pos_mask anyway)
        x_safe_msd  = np.where(x_msd > 1e-10, x_msd_c, 1.0)
        tanh_msd    = np.tanh(x_safe_msd)
        coth_vals   = np.where(x_msd > 1e-10, 1.0 / tanh_msd, 1.0 / x_safe_msd)
        contrib     = np.where(pos_mask[None, :], coth_vals / omega_safe[None, :], 0.0)
        M_mean_SI   = float(np.mean(self.calc.masses_amu)) * AMU_TO_KG
        msd_SI      = (HBAR / (2.0 * M_mean_SI)) * np.sum(contrib, axis=1) / n_q_pts
        MSD_arr     = msd_SI * 1e20   # m² → Å²
        MSD_arr     = np.where(T_arr < 1e-9, MSD_arr[0] if n_T > 0 else 0.0, MSD_arr)
        DW_arr      = 8.0 * np.pi**2 * MSD_arr / 3.0
    ...
    def plot_phonons(
        ...
        auto_title = (
            ...
            else f"{self.label} — SOTC phonons at {self.T_design:.0f} K"
        )
    ...
    def plot_thermal_properties(
        ...
        suptitle = title if title is not None else (
            f"{self.label} — Temperature-dependent properties (SOTC)"
        )
        ...
        # C_V
        ax = axes[0, 0]
        ax.plot(T, Cv, "b-", lw=2, label="SOTC C_V")
        ...
        # S_vib
        ax = axes[0, 1]
        ax.plot(T, S, "b-", lw=2, label="SOTC S_vib")
        _add_ref(ax, "Svib")
        ...
        # F_vib
        ax = axes[1, 0]
        ax.plot(T, F * 1000, "b-", lw=2, label="SOTC F_vib")
        ...
        # T_D(T)
        ax = axes[1, 1]
        ax.plot(T, TD, "b-", lw=2, label="T_D calorimetric (SOTC)")
        ...
        # MSD
        ax = axes[2, 0]
        ax.plot(T, MSD, "b-", lw=2, label="SOTC <u²>")
        ...
        # DW B-factor
        ax = axes[2, 1]
        ax.plot(T, DW, "b-", lw=2, label="SOTC B(T)")
    ...
    def print_summary(
        ...
        print(f"\n{'='*78}")
        print(f"  {self.label}  —  SOTC phonon + thermal summary")
        print(f"{'='*78}")
...
class SOTCRunLoader:
    """
    Load force/displacement data from a completed SOTC work directory.
    ...
    Parameters
    ----------
    work_dir         : path to the SOTC run directory (e.g. ``sqtc_al_fast_vasp_run``).
    ...
    T_design         : SOTC design temperature [K] (read from results.json if None).
    ...
    def _load_results_json(self) -> Optional[Dict]:
        for name in ("sotc_results.json", "sqtc_results.json"):  # compat fallback
    ...
    def build(self) -> "SOTCPostProcessor":
        """
    ...
    def _build_from_vasp(self) -> "SOTCPostProcessor":
        """
        Load POSCAR/OUTCAR snapshots, fit IFCs, build PhononCalculator, and
        return a ready-to-use :class:`SOTCPostProcessor`.
        """
        ...
        for iter_dir in self._iter_dirs_to_load(saved):
            for snap in sorted(iter_dir.glob("snap_*/")):
                ...
                try:
                    ...
                    print(f"  [SOTCRunLoader] Skip {snap.name} (POSCAR): {e}")
                    continue
                ...
                try:
                    ...
                    print(f"  [SOTCRunLoader] Skip {snap.name} (OUTCAR): {e}")
                    continue
    ...
    def _build_from_qe(self) -> "SOTCPostProcessor":
        """
        Load QE ``espresso.pwo`` snapshots, fit IFCs, and return a
        ready-to-use :class:`SOTCPostProcessor`.
        ...
        # When use_all_iters=False, restrict to the last iteration's snaps.
        # With n_ensemble snaps per iteration, only the LAST n_ensemble snaps
        # (from the converged correlator) are used.  Fall back to all snaps if
        # we cannot determine n_ensemble.
        if not self.use_all_iters:
            n_ens = None
            if saved is not None:
                n_ens = saved.get("n_ensemble", None)
            if n_ens is None:
                # Heuristic: use last third of snaps (at least 1)
                n_ens = max(1, len(pwo_dirs) // 3)
                print(f"  [SOTCRunLoader] use_all_iters=False: n_ensemble unknown, "
                      f"using last {n_ens} snaps (heuristic)")
            else:
                print(f"  [SOTCRunLoader] use_all_iters=False: using last {n_ens} snaps "
                      f"(n_ensemble={n_ens} from sotc_results.json)")
            # Always keep snap_0000 (equilibrium) plus last n_ens displaced snaps
            eq_dir  = pwo_dirs[:1]               # snap_0000 = equilibrium
            disp_dirs = pwo_dirs[1:]             # snap_0001 onward
            pwo_dirs = eq_dir + disp_dirs[-n_ens:]
        ...
        for snap_dir in pwo_dirs:
            try:
                ...
                print(f"  [SOTCRunLoader] Skip {snap_dir.name} (pwo): {e}")
        ...
        if _f_max(snap_data[0][1]) > 0.5:  # unexpectedly large → search
            ...
            print(f"  [SOTCRunLoader] Equilibrium snap detected as snap_{eq_idx:04d} "
        ...
        # Use the actual QE equilibrium positions directly (snap_0000 |F|≈0).
        # Reconstructing from prim_cell would give wrong atom ordering whenever
        # the benchmark's prim_pos differs from _build_primitive_cell's choice
        # (e.g. rocksalt O at (a/2,0,0) vs (a/2,a/2,a/2)).  The snap positions
        # are always in the runner's own ordering, which matches all displaced snaps.
        eq_positions = eq_pos
        masses_sc = np.tile(self.masses_amu, n_atoms // n_b)

        # Derive actual primitive-cell basis from the equilibrium snap.
        # The runner's enumeration always places the first n_b atoms at the
        # primitive-cell positions (shift = [0,0,0] is the first tiling step).
        snap_prim_pos = eq_pos[:n_b].copy()
...
def _analytical_r_cutoff(structure: str, a: float) -> float:
    ...
    matches the typical choice in SOTC benchmark runs.
...
def _detect_structure_spglib(sc_cell: np.ndarray,
    ...
    Strategy
    --------
    Displaced SOTC snapshots always have SG #1, so we do NOT use atomic
...
def _auto_detect_run_params(run_dir: Path, structure: str,
    ...
    masses, and T_design from snapshots + sotc_results.json in *run_dir*.

    Supports both VASP (``POSCAR``) and QE (``espresso.pwo``) snapshots.

    Parameters
    ----------
    run_dir        : SOTC run directory.
    ...
    # T_design, r_cutoff, ridge_alpha, symmetrize_bonds from sotc_results.json
    T_design:        Optional[float] = None
    r_cutoff:        Optional[float] = None
    ridge_alpha:     Optional[float] = None
    symmetrize_bonds: Optional[bool] = None
    rjson = next((run_dir / n for n in ("sotc_results.json", "sqtc_results.json") if (run_dir / n).exists()), None)
    if rjson is not None and rjson.exists():
...
def _cli_main():
        ...
        parser = argparse.ArgumentParser(
            description="Post-process a completed SOTC run for any material.",
    ...
    """,
        ...
        parser.add_argument("--run-dir",     required=True,
                            help="Path to the SOTC run directory")
        ...
        parser.add_argument("--T-design",    type=float, default=None,
                            help="SOTC design temperature [K] (auto-read from sotc_results.json if omitted)")
        ...
        parser.add_argument("--symmetrize-bonds", action="store_true", default=None,
                            help="Apply central-force bond symmetrization "
                                 "[auto-read from sotc_results.json if omitted]")
        ...
        parser.add_argument("--use-all-iters", default=True,
                            action=argparse.BooleanOptionalAction,
                            help="Pool snapshots from all iterations (default: True). "
                                 "Use --no-use-all-iters to fit only the last iteration's "
                                 "snaps (best correlator, less data but usually higher R²).")
        ...
        if calculator == "auto":
            ...
            if _has_vasp:
                ...
                calculator = "qe"
            # else: leave as 'auto' — SOTCRunLoader will raise a clear error
        ...
        print(f"\n{'='*68}")
        print(f"  SOTC PostProcessor -- {label}")
        print(f"{'='*68}")
        ...
        loader = SOTCRunLoader(
            ...
            T_design=T_design, use_all_iters=args.use_all_iters,
