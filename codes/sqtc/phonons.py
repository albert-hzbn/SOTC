"""
Phonon dispersion, DOS, and thermodynamic properties from SQTC IFCs.
...
Given the IFC dictionary returned by IFCExtractor.fit(), this module:
  ...
  5. Reconstructs the thermal displacement correlators from DFT-level IFCs
     for use in the next SQTC self-consistency iteration.
...
class PhononCalculator:
    ...
    def frequencies_at_q(
        ...
        # Convert eigenvalues ω² → ω (sign-safe)
        omega = np.where(w2 >= 0, np.sqrt(w2), -np.sqrt(-w2))
        if return_eigvecs:
            return omega, vecs
        return omega
    ...
    def heat_capacity_scan(
        ...
        for it, T in enumerate(T_values):
            ...
            sinh_x = np.sinh(x_c)
            # Classical limit when x → 0: (x/sinh x)² → 1
            cv_per_mode = np.where(x < 1e-10, 1.0, (x / sinh_x) ** 2)
    ...
    def vibrational_free_energy(
        ...
        Used by SQTCQuasiHarmonic to minimise G(T,P,V) = E_0(V) + F_vib(T,V)
        over volume, giving thermal expansion without extra DFT beyond the
        SQTC force evaluations themselves.
        """
        n_q = q_mesh[0] * q_mesh[1] * q_mesh[2]
        all_omegas = self._all_frequencies(q_mesh).ravel()
        omegas = all_omegas[all_omegas > self._imag_tol]
    ...
    # ── Updated correlators for SQTC self-consistency ─────────────────────────
    ...
    def updated_correlators(
        ...
        Used in the SQTC self-consistency loop to replace the initial Debye
    ...
    def quantum_displacement_amplitudes(
        ...
        Use in the SQTC runner instead of classical Q = √(k_BT)/ω to
