"""
SQTC self-consistent runner.

Implements the full SQTC algorithm loop (Section 3 of SQTC_framework.md):

  Step 1  Compute target correlators C̄₂^target(R,T)  [Debye or phonopy]
  Step 2  Enumerate HNF supercell candidates
  Step 3  Optimise displacement fields via SA to minimise Q_SQTC
...
class SQTCRunner:
    """
    Orchestrates the full SQTC self-consistent loop.
    ...
    n_atoms_sc : int
        Target number of atoms in the SQTC supercell.
    ...
    r_max_corr : float [Å]
        Maximum shell distance included in Q_SQTC.
    n_ensemble : int
        Number of SQTC displacement configurations.
    work_dir : str or Path
        Working directory for SQTC calculations.
    verbosity : int
    ...
    def _optimise_ensemble(
        ...
        """Run SA to produce n_ensemble SQTC displacement configurations."""
    ...
    def _evaluate_forces(
        ...
        """
        Evaluate DFT (or mock) forces for all SQTC configurations in parallel.
    ...
    def run(
        ...
        """
        Run the full SQTC self-consistency loop.
        ...
        if self.verbosity >= 1:
            ...
            print(f"\n{'='*60}")
            print(f"SQTC run: {formula}, T={self.T} K, T_D={self.T_D} K")
            print(f"{'='*60}")
        ...
        summary = {k: v for k, v in self._results.items()
                   if k not in ("phonon_calculator", "ifc_extractor")}
        ...
        (self.work_dir / "sqtc_results.json").write_text(
...
class _InterpolatedCorrelator:
    ...
    drop-in for DebyeCorrelator in subsequent SQTC iterations.
    """
